...
**계정을 바꾸거나 다시 만들고 싶을 때도 같은 명령입니다.**

```bash
# 1) .env.local 에서 이메일을 고친다
#    ADMIN_EMAIL=admin@caredoc.com
# 2) 다시 실행
npm run seed:admin
```

- 그 이메일의 계정이 **없으면 새로 만들고**, **있으면 비밀번호를 초기값으로 되돌립니다.**
  (비밀번호를 잊었을 때도 이 명령으로 풀 수 있습니다)
- 어느 쪽이든 최초 로그인 시 비밀번호 변경 화면이 다시 강제됩니다.
- 예전 계정은 자동으로 지워지지 않습니다. 정리하려면 Supabase 대시보드 →
  **Authentication → Users** 에서 삭제하세요.
...
```
이메일    admin@caredoc.com
비밀번호  admin123
```

> ⚠ 최초 로그인하면 **비밀번호 변경 화면이 강제로** 뜹니다.
> `admin123` 은 매우 취약한 값이므로, 실제 환자 정보를 넣기 전에 반드시 변경하세요.
> 새 비밀번호에 자체 제한은 두지 않았습니다. Supabase 가 요구하는 **6자 이상**만 지키면 됩니다.
...
| `npm run lint` | 코드 검사 |
| `npm run seed:admin` | 관리자 계정 생성 · 비밀번호 초기화 (`.env.local` 의 `ADMIN_EMAIL` 기준) |
...
### 잘못 만든 평가 지우기
평가를 잘못 눌러 만들었다면, 환자 화면의 **평가 이력**에서 그 줄 오른쪽 휴지통을
누르거나 평가 작성 화면 맨 아래의 휴지통을 누르세요.
**작성 중인 평가만** 지울 수 있습니다 — 완료한 평가는 진료기록이라 지우지 않고,
고칠 내용이 있으면 수정해서 새 버전으로 남깁니다.
